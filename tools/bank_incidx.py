#!/usr/bin/env python3
"""Auto-decompile constant-increment accessors on a pointer-field array (24 bytes):

    lwz rB, C(r3) ; slwi r4, i, 2 ; lwzx r3, rB, r4 ; addi r0, r3, K ; stwx r0, rB, r4 ; blr

i.e. arr[i] += K with a compile-time constant K (typically ++/--), where
arr = (int*)*(void**)((char*)p + C), i in r4. Structure is fixed except the
field offset C and the immediate K.

Usage: python3 tools/bank_incidx.py <module> [max_functions]
"""
...
def classify(a, sz):
    ...
    if w[5] != 0x4E800020:
        return None
    if (w[0] >> 26) != 32 or ((w[0] >> 16) & 31) != 3:              # lwz rB, C(r3)
        return None
    rB = (w[0] >> 21) & 31
    if w[1] != 0x5484103A:                                          # slwi r4, r4, 2
        return None
    if w[2] != (31 << 26 | 3 << 21 | rB << 16 | 4 << 11 | 23 << 1):  # lwzx r3, rB, r4
        return None
    if (w[3] >> 26) != 14 or ((w[3] >> 21) & 31) != 0 or ((w[3] >> 16) & 31) != 3:  # addi r0, r3, K
        return None
    if w[4] != (31 << 26 | 0 << 21 | rB << 16 | 4 << 11 | 151 << 1):  # stwx r0, rB, r4
        return None
    return sign16(w[0] & 0xFFFF), sign16(w[3] & 0xFFFF)
...
for line in open(os.path.join(ROOT, "config/RSBE01_02/rels", module, "symbols.txt")):
    ...
    src = (f"void {n}(void* p, int i) {{\n"
           f"    ((int*)*(void**)((char*)p + {off}))[i] += {k};\n}}\n")
    add_unit_rel.add(module, f"mo_stub/{module}/ii_{n}.c", a, a + sz, src)
    addrs.append(f"{module}:{a:08X}")
    banked += 1

open(LAST, "w").write("\n".join(addrs) + "\n")
print(f"[{module}] banked {banked} constant-increment accessors")
