#!/usr/bin/env python3
"""Auto-decompile consecutive-float setters:

    stfs f1, o0(r3) ; stfs f2, o1(r3) ; ... ; stfs fN, oN(r3) ; blr

storing N float parameters into N fields of one object (e.g. a Vec2/Vec3 setter).
N ranges over 2..8; offsets are read from the instructions.

Usage: python3 tools/bank_fset.py <module> [max_functions]
"""
...
def classify(a, sz):
    ...
    if w[-1] != 0x4E800020:
        return None
    offs = []
    for k in range(n):                                 # stfs f(k+1), o(r3)
        if (w[k] >> 26) != 52 or ((w[k] >> 21) & 31) != k + 1 or ((w[k] >> 16) & 31) != 3:
            return None
...
for line in open(os.path.join(ROOT, "config/RSBE01_02/rels", module, "symbols.txt")):
    ...
    if not offs:
        continue
    params = ", ".join(f"float a{k}" for k in range(len(offs)))
    body = "".join(f"    *(float*)((char*)p + {o}) = a{k};\n" for k, o in enumerate(offs))
    src = f"void {n}(void* p, {params}) {{\n{body}}}\n"
    add_unit_rel.add(module, f"mo_stub/{module}/fs_{n}.c", a, a + sz, src)
    addrs.append(f"{module}:{a:08X}")
    banked += 1

open(LAST, "w").write("\n".join(addrs) + "\n")
print(f"[{module}] banked {banked} consecutive-float setters")
