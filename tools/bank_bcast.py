#!/usr/bin/env python3
"""Auto-decompile broadcast-int setters:

    stw r4, o0(r3) ; stw r4, o1(r3) ; ... ; stw r4, oN(r3) ; blr

storing one int parameter into N fields of one object. N ranges over 2..7.

Usage: python3 tools/bank_bcast.py <module> [max_functions]
"""
...
def classify(a, sz):
    ...
    if w[-1] != 0x4E800020:
        return None
    offs = []
    for k in range(n):                             # stw r4, o(r3) -- same source r4
        if (w[k] >> 26) != 36 or ((w[k] >> 21) & 31) != 4 or ((w[k] >> 16) & 31) != 3:
            return None
        offs.append(sign16(w[k] & 0xFFFF))
    if len(set(offs)) != len(offs):                # distinct fields
        return None
    return offs
...
for line in open(os.path.join(ROOT, "config/RSBE01_02/rels", module, "symbols.txt")):
    ...
    if not offs:
        continue
    body = "".join(f"    *(int*)((char*)p + {o}) = v;\n" for o in offs)
    src = f"void {n}(void* p, int v) {{\n{body}}}\n"
    add_unit_rel.add(module, f"mo_stub/{module}/bc_{n}.c", a, a + sz, src)
    addrs.append(f"{module}:{a:08X}")
    banked += 1

open(LAST, "w").write("\n".join(addrs) + "\n")
print(f"[{module}] banked {banked} broadcast-int setters")
